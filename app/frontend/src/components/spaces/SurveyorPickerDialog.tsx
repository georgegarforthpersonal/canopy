/**
 * The editors'/admins' surveyor picker for an upcoming survey: an inline
 * instant-apply list — toggling a name PUTs the new surveyor_ids onto the
 * survey (optimistically, serialized so rapid taps can't race, reverted to
 * the last server-acknowledged set on failure). No Save/Cancel pair — just
 * Done. The space updates the row's avatars from the returned ids.
 *
 * Viewers never see this dialog — their sign-up is the one-click
 * SelfSignupButton rendered directly in the survey row.
 */
...
export default function SurveyorPickerDialog({ open, survey, surveyors, onClose, onSaved }: SurveyorPickerDialogProps) {
