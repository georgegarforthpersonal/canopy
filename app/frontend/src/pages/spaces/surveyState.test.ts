...
import { deriveSurveyState, buildWorklist, nextScheduledSurvey, formatWeekRange } from './surveyState';
...
describe('buildWorklist', () => {
  ...
  it('places a due-this-week (in-window) survey in the needs-survey bucket, not upcoming', () => {
    ...
    const { needsSurvey, upcoming } = buildWorklist([weekly], TODAY);
    expect(needsSurvey.map((s) => s.id)).toEqual([20]);
